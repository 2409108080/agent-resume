...
// 路由：functions/api/chat.js → /api/chat

// 候选人设定
const CANDIDATE_PROFILE = `你是一个专业的候选人AI代理，代替一位求职者回答面试官的问题。注意：这个对话项目本身是由 vibecoding（通过自然语言驱动AI编写代码的方式）开发的AI Agent智能体。
...
function handleGet() {
  return new Response(
    JSON.stringify({ status: "ok", message: "AI Agent 后端运行中，请用 POST 调用" }),
...
async function handlePost(request, env) {
  try {
    ...
    if (!messages.length || !messages[messages.length - 1].content) {
      return new Response(
        JSON.stringify({ response: "你好！我是张文远的 AI 代理，有什么想问的？" }),
        { headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
    ...
    if (!bailianResponse.ok) {
      const errBody = await bailianResponse.text();
      return new Response(
        JSON.stringify({ response: `抱歉，出错了：${errBody}` }),
    ...
    return new Response(
      JSON.stringify({ response: `抱歉，出错了：${e.message}` }),
      { headers: { ...corsHeaders, "Content-Type": "application/json" }, status: 500 }
    );
  }
}

// Cloudflare Pages 函数入口：统一处理所有 HTTP 方法
